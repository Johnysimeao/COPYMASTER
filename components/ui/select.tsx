    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn("relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-[280px] origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-xl bg-popover text-popover-foreground shadow-2xl ring-1 ring-foreground/10 duration-100 data-[align-trigger=true]:animate-none", className )}
          {...props}
        >
          <SelectScrollUpButton className="bg-[#1A1A1C] border-b border-white/5" />
          <SelectPrimitive.List>{children}</SelectPrimitive.List>
          <SelectScrollDownButton className="bg-[#1A1A1C] border-t border-white/5" />
        </SelectPrimitive.Popup>
      </SelectPrimitive.Positioner>
    </SelectPrimitive.Portal>
  )
